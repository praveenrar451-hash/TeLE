...
// ==========================================
// CHAT FEATURES: TYPING, STATUS, LAST SEEN, SEEN/UNSEEN & TIMESTAMPS
// ==========================================

let typingTimeout = null;
let currentChatChannel = null;

// 1. Initialize Realtime Chat Status & Typing Channel
window.initChatRealtimeFeatures = function(recipientUsername) {
    if (!supabaseClient) return;
    
    const currentUser = localStorage.getItem('currentUsername');
    if (!currentUser) return;

    const roomName = `chat_status_${[currentUser, recipientUsername].sort().join('_')}`;
    
    if (currentChatChannel) {
        supabaseClient.removeChannel(currentChatChannel);
    }

    currentChatChannel = supabaseClient.channel(roomName, {
        config: { broadcast: { self: false } }
    });
    ...
    currentChatChannel
        .on('broadcast', { event: 'typing' }, ({ payload }) => {
            ...
            if (indicatorEl) {
                if (payload.isTyping && payload.sender === recipientUsername) {
                    indicatorEl.style.display = 'block';
                    indicatorEl.textContent = `${recipientUsername} is typing...`;
                } else {
                    indicatorEl.style.display = 'none';
                }
            }
        })
        .on('broadcast', { event: 'message-seen' }, ({ payload }) => {
            // Update message tick/status to seen in UI if needed
            updateMessageSeenUI(payload.messageId);
        })
        .subscribe();
};

// 2. Trigger Typing Indicator (Call this on message input keyup)
window.sendTypingIndicator = function(isTyping, recipientUsername) {
    if (!currentChatChannel) return;
    const currentUser = localStorage.getItem('currentUsername');
    
    currentChatChannel.send({
        type: 'broadcast',
        event: 'typing',
...
// 3. Format Message Timestamp (Kis din aur kis time bheja gaya)
...
// 4. Update Online Status & Last Seen in Database
window.updateUserPresence = async function(isOnline) {
    const currentUser = localStorage.getItem('currentUsername');
    if (!currentUser) return;
...
// Set online when active, offline when closing/leaving page
...
// 5. Format Last Seen Text
window.formatLastSeen = function(lastSeenString) {
...
// Helper for Seen UI update
function updateMessageSeenUI(msgId) {
    const tickEl = document.getElementById(`msg-tick-${msgId}`);
    if (tickEl) {
        tickEl.innerHTML = '<i class="fa-solid fa-check-double" style="color: #3897f0;"></i>'; // Blue/Seen ticks
    }
}
