...
tabReelsBtn.onclick = () => {
            ...
            userProfileReels.style.display = 'grid';
            userProfilePosts.style.display = 'none';
            fetchAndRenderUserReels(username);
        };
    }
}
...
async function fetchUserProfilePosts(username) {
    if (!userProfilePosts) return;
    userProfilePosts.innerHTML = '<p style="color:#777; text-align:center; grid-column:1/-1; margin-top:20px;">Loading...</p>';
    ...
    if (error) {
        userProfilePosts.innerHTML = '<p style="color:#777; text-align:center; grid-column:1/-1; margin-top:20px;">Failed to load posts.</p>';
        return;
    }

    userProfilePosts.innerHTML = '';
    if (!data || data.length === 0) {
        userProfilePosts.innerHTML = '<p style="color:#777; text-align:center; grid-column:1/-1; margin-top:20px;">No posts yet.</p>';
        return;
    }

    data.forEach(post => {
        const postThumb = document.createElement('div');
        postThumb.style.cssText = "width:100%; aspect-ratio:1/1; background:#111; position:relative; cursor:pointer; overflow:hidden;";
        postThumb.innerHTML = `<img src="${post.image_url}" style="width:100%; height:100%; object-fit:cover;">`;
        userProfilePosts.appendChild(postThumb);
    });
}

// Fetch and Render User Reels
async function fetchAndRenderUserReels(username) {
    if (!userProfileReels) return;
    userProfileReels.innerHTML = '<p style="color:#777; text-align:center; grid-column:1/-1; margin-top:20px;">Loading...</p>';
    ...
    if (error) {
        userProfileReels.innerHTML = '<p style="color:#777; text-align:center; grid-column:1/-1; margin-top:20px;">Failed to load reels.</p>';
        return;
    }

    userProfileReels.innerHTML = '';
    if (!data || data.length === 0) {
        userProfileReels.innerHTML = '<p style="color:#777; text-align:center; grid-column:1/-1; margin-top:20px;">No reels yet.</p>';
        return;
    }

    data.forEach(reel => {
        const reelThumb = document.createElement('div');
        reelThumb.style.cssText = "width:100%; aspect-ratio:9/16; background:#111; position:relative; cursor:pointer; overflow:hidden;";
        reelThumb.innerHTML = `<video src="${reel.video_url}" style="width:100%; height:100%; object-fit:cover;"></video>`;
        userProfileReels.appendChild(reelThumb);
    });
}

// ==========================================
// EDIT PROFILE & FOLLOWERS MODAL SYSTEM
// ==========================================
function openEditProfileModal(username) {
    let existingModal = document.getElementById('edit-profile-modal');
    if (existingModal) existingModal.remove();

    const modal = document.createElement('div');
    modal.id = 'edit-profile-modal';
    modal.style.cssText = "position:fixed; top:0; left:0; width:100%; height:100%; background:rgba(0,0,0,0.7); z-index:999999; display:flex; justify-content:center; align-items:center;";
    
    modal.innerHTML = `
        <div style="background:#121212; padding:20px; border-radius:12px; width:90%; max-width:320px; border:1px solid #333; text-align:center;">
            <h3>Edit Profile</h3>
            <p style="font-size:12px; color:#aaa; margin-bottom:15px;">Paste your profile picture image URL below:</p>
            <input type="text" id="edit-avatar-url-input" placeholder="https://example.com/image.jpg" style="width:100%; padding:10px; background:#1a1a1a; border:1px solid #444; color:#fff; border-radius:6px; margin-bottom:15px; font-size:13px; box-sizing:border-box;">
            <div style="display:flex; gap:10px;">
                <button id="save-profile-btn" style="flex:1; background:#0095f6; color:#fff; border:none; padding:10px; border-radius:6px; font-weight:bold; cursor:pointer;">Save</button>
                <button id="cancel-profile-btn" style="flex:1; background:#333; color:#fff; border:none; padding:10px; border-radius:6px; cursor:pointer;">Cancel</button>
            </div>
        </div>
    `;
    ...
    document.getElementById('cancel-profile-btn').onclick = () => modal.remove();
    document.getElementById('save-profile-btn').onclick = async () => {
        const newUrl = document.getElementById('edit-avatar-url-input').value.trim();
        if (newUrl) {
            await supabaseClient.from('users').update({ avatar_url: newUrl }).eq('username', username);
            modal.remove();
            openProfilePage(username, true);
        }
    };
}

async function openFollowListModal(username, type) {
    let existingModal = document.getElementById('follow-list-modal');
    if (existingModal) existingModal.remove();
    ...
    modal.innerHTML = `
        <div style="background:#121212; width:90%; max-width:320px; height:60%; border-radius:12px; border:1px solid #333; display:flex; flex-direction:column; overflow:hidden;">
            <div style="display:flex; justify-content:space-between; align-items:center; padding:12px 15px; border-bottom:1px solid #262626;">
                <h4 style="margin:0; text-transform:capitalize;">${type}</h4>
                <button id="close-follow-modal" style="background:transparent; border:none; color:#fff; font-size:16px; cursor:pointer;"><i class="fa-solid fa-xmark"></i></button>
            </div>
            <div id="follow-list-content" style="flex:1; overflow-y:auto; padding:10px 15px; display:flex; flex-direction:column; gap:10px;">
    ...
    document.body.appendChild(modal);
    document.getElementById('close-follow-modal').onclick = () => modal.remove();
    ...
    if (type === 'followers') {
        ...
        if (data) targetUsers = data.map(item => item.follower);
    } else {
        const { data } = await supabaseClient.from('follows').select('following').eq('follower', username);
        if (data) targetUsers = data.map(item => item.following);
    }

    if (targetUsers.length === 0) {
        contentDiv.innerHTML = `<p style="color:#777; text-align:center; margin-top:20px;">No ${type} yet.</p>';`;
        return;
    }

    const { data: usersData } = await supabaseClient.from('users').select('username, avatar_url').in('username', targetUsers);
        ...
        usersData.forEach(user => {
            ...
            row.innerHTML = `
                <div style="display:flex; align-items:center; gap:10px;">
                    <div style="width:36px; height:36px; background:#444; border-radius:50%; display:flex; align-items:center; justify-content:center; font-weight:bold; overflow:hidden; ${user.avatar_url ? `background-image:url(${user.avatar_url}); background-size:cover;` : ''}">${!user.avatar_url ? user.username.charAt(0).toUpperCase() : ''}</div>
                    <b>${user.username}</b>
                </div>
            `;
            row.onclick = () => {
                modal.remove();
                switchView('user-profile-container');
                openProfilePage(user.username, false);
            };
